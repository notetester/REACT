...
!!! tip "처음 방문했다면"
    이 사이트가 순서대로 읽는 **온라인 교재**입니다. 왼쪽 내비게이션에서 **React → Spring Boot → 연동(최종 응용)** 순서로 읽으세요. React 노트에는 설명과 코드 바로 위에 조작 가능한 결과 화면을 넣었습니다. 처음에는 설치 없이 아래 라이브 데모를 둘러보고, 코드를 직접 수정할 때 저장소를 내려받으면 됩니다.
