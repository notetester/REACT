...
!!! tip "처음 방문했다면"
    이 사이트가 순서대로 읽는 **온라인 교재**입니다. 왼쪽 내비게이션에서 **React → Spring Boot → 연동(최종 응용)** 순서로 읽으세요. React 노트에는 설명과 코드 바로 아래에 조작 가능한 결과 화면을 넣었습니다. 처음에는 설치 없이 아래 라이브 데모를 둘러보고, 코드를 직접 수정할 때 저장소를 내려받으면 됩니다.
...
| 2 | [React 단계별 결과 탐색기](/REACT/demo/react-basics/#/lab)와 [연동 mock 데모](/REACT/demo/integration/)를 열어 화면을 직접 조작합니다. 연동 데모는 `study` / `1111`로 로그인합니다. | 없음 |
...
## 🚀 라이브 데모
- **[React 단계별 결과 탐색기 (my-app01)](/REACT/demo/react-basics/#/lab)** — JSX·state·form·Hooks·Context·useReducer를 노트와 함께 즉시 조작
